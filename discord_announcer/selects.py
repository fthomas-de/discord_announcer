...
def get_corp_transaction_token(corp_id: int) -> Token:
    if corp_id is None:
        logger.error("[discord_announcer] Corporation ID is None")
        return False
    ...
    if not token:
        logger.error("[discord_announcer] No token found with required scope: %s", REQUIRED_SCOPE)
        return False
    return token
...
def get_transactions(corp_id: int, division: int):
    ...
    if not transactions:
        logger.info("[discord_announcer] No transactions found for corporation: %s", corp_id)
        return None
...
def get_transactions_for_timeframe(time: int, corporation_id: int, division: int):
    ...
    if transactions is None:
        logger.info("[discord_announcer] No transactions found")
        return []
